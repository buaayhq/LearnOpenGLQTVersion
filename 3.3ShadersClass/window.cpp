...
//  connect(context(), SIGNAL(aboutToBeDestroyed()), this, SLOT(teardownGL()), Qt::DirectConnection);
  ...
  //添加顶点着色器
  pShaderProgram->addShaderFromSourceFile(QOpenGLShader::Vertex, "3.3.shader.vs");
  //添加片段着色器
  pShaderProgram->addShaderFromSourceFile(QOpenGLShader::Fragment, "3.3.shader.fs");
  ...
  // 简单来说就是一个顶点属性组的字节大小
  pShaderProgram->setAttributeBuffer(0, GL_FLOAT, 0, 3, 6*sizeof(float));
  pShaderProgram->setAttributeBuffer(1, GL_FLOAT, 3*sizeof(float), 3, 6*sizeof(float));
