...
void Window::paintGL()
{
  ...
  //pVAO->bind(); // only one VAO, it was bound already.

  QColor ourColor(0.0f, 0.0f, 0.0f);

  ourColor.setGreenF(sin(currentTime.elapsed()/1000.f)/2.0f + 0.5f);
  pShaderProgram->setUniformValue("ourColor", ourColor);
