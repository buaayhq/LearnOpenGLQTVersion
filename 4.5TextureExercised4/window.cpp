...
,pTexture1(nullptr),pTexture2(nullptr), mixValue(0.2)
{
...
//  connect(context(), SIGNAL(aboutToBeDestroyed()), this, SLOT(teardownGL()), Qt::DirectConnection);
  ...
  //添加顶点着色器
  pShaderProgram->addShaderFromSourceFile(QOpenGLShader::Vertex, "4.5.texture.vs");
  //添加片段着色器
  pShaderProgram->addShaderFromSourceFile(QOpenGLShader::Fragment, "4.5.texture.fs");
  ...
  // texture1
  pTexture1 = new QOpenGLTexture(QOpenGLTexture::Target2D);
  pTexture1->create();
  pTexture1->bind();
  ...
  pTexture1->setMinMagFilters(QOpenGLTexture::Linear, QOpenGLTexture::Linear);
  pTexture1->setData(QImage(QString("container.jpg")).mirrored());

  // texture2
  pTexture2 = new QOpenGLTexture(QOpenGLTexture::Target2D);
  pTexture2->create();
  pTexture2->bind();
  ...
  pTexture2->setMinMagFilters(QOpenGLTexture::Linear, QOpenGLTexture::Linear);
  pTexture2->setData(QImage(QString("awesomeface.png")).mirrored());

  //在setUniformValue之前需要激活shader program
  pShaderProgram->bind();
  pShaderProgram->setUniformValue("texture1", 0);
  pShaderProgram->setUniformValue("texture2", 1);
...
void Window::paintGL()
{
  ...
  pShaderProgram->bind(); // set uniform之前激活
  pShaderProgram->setUniformValue("mixValue", mixValue);
  qDebug() << mixValue;
